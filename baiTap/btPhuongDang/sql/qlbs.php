...
<body>
    ...
    $sql = 'SELECT s.ten_sua, h.ten_hang_sua, l.ten_loai, s.Trong_luong, s.don_gia
    FROM sua AS s
    ...
    <th width="50">Hãng sữa</th>
    <th width="50">Loại sữa</th>
    <th width="80">Trọng lượng</th>
    <th width="90">Đơn giá</th>
</tr>';
    // Phân trang
    $numberrow = 5; // Số dòng dữ liệu tối đa trên mỗi trang
    ...
    if (mysqli_num_rows($paginationResult) <> 0) {
        ...
        while ($rows = mysqli_fetch_row($paginationResult)) {
            if ($stt % 2 == 0) {
                echo "<tr style=' color:#A73121;background-color:#80B3FF'>";
                echo "<td>$stt</td>";
                echo "<td>$rows[0]</td>";
                echo "<td>$rows[1]</td>";
                echo "<td>$rows[2]</td>";
...
>
        ...
        // Gắn thêm nút quay về trang cuối
        if ($_GET['page'] < $totalPages) {
